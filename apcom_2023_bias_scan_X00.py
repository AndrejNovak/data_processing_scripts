...
if os.path.isfile(PathOut + '\\number_of_single_pixel_counts_bias_scan_X00.txt'):
    ...
    plt.title('TPX2 X00 Si 500 $\mu$m, bias scan')
...
for i in range(len(subdirectories)-1):
    ...
    plt.tick_params(axis='x', labelsize=tickfnt)
    plt.tick_params(axis='y', labelsize=tickfnt)
    plt.title(f'TPX2 X00 Si 500 $\mu$m, bias {i}V')
    ...
    histogram_data = np.loadtxt(PathOut + '\\histograms\\X00_tpx2_histogram_data_'+str(i)+'V.txt')
...
plt.tick_params(axis='x', labelsize=tickfnt)
plt.tick_params(axis='y', labelsize=tickfnt)
plt.title('Am-241 59.6 keV peak bias scan, TPX2 X00 Si 500 $\mu$m')
plt.savefig(PathOut + '\\X00_tpx2_total_counts_Am241_different_voltage.png', dpi=300, transparent=True, bbox_inches="tight", pad_inches=0.01)  