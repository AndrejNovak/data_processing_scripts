...
FileInPath = 'Q:\\DPE_carlos_data_output\\2022_12_VdG\\L07\\11\\'
FolderOut = 'Q:\\2023_iworid_data_processing\\processing_selected\\VdG_11\\L07\\'
...
filter_parameters = Cluster_filter_multiple_parameter([E_min, E_max, H_min, H_max, Size_min, Size_max], [4,7,8]) # Energy
...
FileOutName = 'E_' + str(E_min) + '-' + str(E_max) + '_H_' + str(H_min) + '-' + str(H_max) + '_Size_' + str(Size_min) + '-' + str(Size_max)
...
number_of_single_clusters = 100
passed_clusters = 0

for i in range(len(filtered_elist[:,0])):
    if filtered_elist[i,-1] == 1 and passed_clusters < number_of_single_clusters:
        #print(clog[i])
        print_figure_single_cluster_energy(clog[i], i, energy_colorbar_max_value, 'Cluster #' + str(i), FolderOut + '\\' + FileOutName + '\\', 'Cluster')
        passed_clusters +=1